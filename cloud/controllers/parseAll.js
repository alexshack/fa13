...
function parseAll(allText) {
    ...
    var flagsQuery = Parse.Query("Flags");
    ...
    flagsQuery.find({
        ...
        success: function (flags) {
            ...
            var turnirQuery = Parse.Query("Flags");
            flagsQuery.find({
                success: function (turnirs) {
                    ...
                    var iAll = parseTurnir(arr, turnirsObj, turnirs) + 2;

                    continueParseAll(flags, iAll, turnirs)
                },
                error: function (error) {
                    console.log("Error on getting turnirs: " + error.code + " " + error.message);
                    // res.send(error);
                    status.error("Error on getting turnirs: " + error.code + " " + error.message);
                }
            });
    ...
    function continueParseAll(flags, iAll, turnirs) {

        console.log(iAll + '-' + arr[iAll]);

        while (iAll < arr.length) {
            iAll = parseClub(flags, arr, iAll, allDate, clubs, managers, players, turnirs);
            iAll++;
        }
...
function parseClub(flags, arr, i, date, clubs, managers, players, turnirs) {
